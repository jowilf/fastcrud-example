...
class UserAdmin(BaseAdminModel):
    ...
    def create(self, form_data: FormData):
        ...
        if self.rm.user.find_one(UserFilter(username=user_in.username)) is not None:
            raise HTTPException(
                HTTP_409_CONFLICT,
        ...
        self.rm.user.save(user)

    def edit(self, form_data: FormData, id):
        _data = self._extract_fields(form_data, True)
        user = self.rm.user.find_by_id(id)
        user_in = UserPatchBody(**_data)
        if (
            user.username != user_in.username
            and self.rm.user.find_one(UserFilter(username=user_in.username)) is not None
        ):
            raise HTTPException(
                HTTP_409_CONFLICT,
                detail=[{"loc": ["username"], "msg": f"username already exist."}],
